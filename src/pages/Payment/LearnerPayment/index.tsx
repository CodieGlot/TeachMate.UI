...
export function LearnerPayment() {
    ...
    const HandlePayForClass = async () => {
        try {
           ...
           if (selectedPayment == "zalo" && UnPaidPayment) {
            const response = await PaymentService.createZaloPayOrder(UnPaidPayment?.paymentAmount)
            window.open(response.orderUrl, '_blank');  // Open in a new tab
            console.log(response);
           }
           if (selectedPayment == "vnpay" && UnPaidPayment) {
            const response = await PaymentService.createVnPayOrder(UnPaidPayment?.paymentAmount)
            window.open(response.orderUrl, '_blank');  // Open in a new tab
            console.log(response);
           }
           if (selectedPayment == "momo" && UnPaidPayment) {
            const response = await PaymentService.createMomoPayOrder(UnPaidPayment?.paymentAmount)
            window.open(response.orderUrl, '_blank');  // Open in a new tab
            console.log(response);
           }
           console.log(selectedPayment == "zalo")
           console.log("hello")
        } catch (error) {
                        ...
                        {UnPaidPayment ? (
                            <div className="mt-8 flex gap-3 justify-center">
                                ...
                                <div className="p-10 bg-white w-[300px] h-[400px] aspect-square rounded-lg shadow flex flex-col items-center justify-center gap-2 ">
                                    ...
                                    <label
                                        className={`inline-flex justify-between w-full items-center z-10 rounded-lg p-2 border transition-all cursor-pointer ${selectedPayment === 'zalo'
                                    ...
                                    >
                                        <div className="inline-flex items-center justify-center gap-2 relative z-10">
                                            ...
                                            <p
                                                className={`font-semibold absolute inset-0 w-full whitespace-nowrap ${selectedPayment === 'zalo'
                                    ...
                                    <label
                                        className={`inline-flex justify-between w-full items-center z-10 rounded-lg p-2 border transition-all cursor-pointer ${selectedPayment === 'vnpay'
                                            ? 'border-indigo-500 text-indigo-900 bg-indigo-50 font-bold hover:bg-slate-200 duration-500'
                                            : 'border-transparent'
                                            }`}
                                    >
                                        <div className="inline-flex items-center justify-center gap-2 relative z-10">
                                            ...
                                            <p
                                                className={`font-semibold absolute inset-0 w-full whitespace-nowrap ${selectedPayment === 'vnpay'
                                                    ? 'translate-y-[0%] translate-x-full top-1 left-2 opacity-100 duration-700'
                                                    : '-translate-y-[110%] translate-x-full top-1 left-2 opacity-0 duration-700'
                                                    }`}
                                            >
                                    ...
                                    <label
                                        className={`inline-flex justify-between w-full items-center z-10 rounded-lg p-2 border transition-all cursor-pointer ${selectedPayment === 'momo'
                                            ? 'border-indigo-500 text-indigo-900 bg-indigo-50 font-bold hover:bg-slate-200 duration-500'
                                            : 'border-transparent'
                                            }`}
                                    >
                                        <div className="inline-flex items-center justify-center gap-2 relative z-10">
                                            ...
                                            <p
                                                className={`font-semibold absolute inset-0 w-full whitespace-nowrap ${selectedPayment === 'momo'
                                                    ? 'translate-y-[0%] translate-x-full top-1 left-2 opacity-100 duration-700'
                                                    : '-translate-y-[110%] translate-x-full top-1 left-2 opacity-0 duration-700'
                                                    }`}
                                            >
