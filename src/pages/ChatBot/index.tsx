...
export function ChatBot() {
    ...
    const [qaList, setQaList] = useState<QAObject[]>([]);
    const handleQuestionChatBot = async () => {
        try {
            const data = await ChatBotService.questionChatBot({
                question
            });
            setAnswer(data);
            setQaList([...qaList, { question, answer: data }]);
            setQuestion('');
        } catch (err) {
    ...
    return (
        <>
            ...
            {
                open && (
                    <div style={{ boxShadow: "0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgba(0, 0, 0, 0.05)" }}
                        className="sticky left-[100%] bottom-[calc(4rem+1.5rem)] right-0 mr-4 bg-white p-6 rounded-lg border border-[#e5e7eb] w-[440px] h-[500px]">

                        <div className="flex flex-col space-y-1.5 pb-6">
                            <h2 className="font-semibold text-lg tracking-tight">Chatbot</h2>
                            <p className="text-sm text-[#6b7280] leading-3">Ask your problem with AI</p>
                        </div>




                        <div className="overflow-y-auto pr-4 h-[350px]" style={{ minWidth: "100%", overflowY: "auto" }}>
                            <div className="flex gap-3 my-4 text-gray-600 text-sm flex-1"><span
                                className="relative flex shrink-0 overflow-x-hidden rounded-full w-8 h-8">
                            ...
                            {qaList.map((qa, index) => (
                                <div className="qa">
                                    <div className="flex gap-3 my-4 text-gray-600 text-sm flex-1"><span
                                        ...
                                        <p className="leading-relaxed"><span className="block font-bold text-gray-700">You </span>{qa.question}</p>
                                    </div>
